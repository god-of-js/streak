...
router.get('/get-series',
    isAuthenticated,
    make(movies.getSeries)
    );
router.get('/get-movies',
    isAuthenticated,
    make(movies.getMovies)
    );
router.get('/get-recently-added-movies',
        isAuthenticated,
        make(movies.getRecentlyAddedMovies)
    );
router.get('/get-recently-added-series',
        isAuthenticated,
        make(movies.getRecentlyAddedSeries));
router.get('/get-single-movie',
            isAuthenticated,
            make(movies.getSingleMovie))
router.get('/get-single-serie',
            isAuthenticated,
            isAccountType('admin'),
            make(movies.getSingleSerie))
